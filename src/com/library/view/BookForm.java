...
public class BookForm extends javax.swing.JInternalFrame {
    ...
    public BookForm() {
        initComponents();
        getBookListFromDao();
        getBookCategoryListFromDao();
        displayBooks();
        displayBookCategories();
        updateBookCategoryComboBox();
    }
    
    public void getBookListFromDao() {
        bookList = bookActions.retrieveAll(book);
    }
    
    public void getBookCategoryListFromDao() {
    ...
    private void displayBooks() {
        ...
        for (Book aBk : bookList) {
            bookmodel.addRow(new Object[]{
                aBk.getBookId(),
                aBk.getTitle(),
                aBk.getAuthor(),
                aBk.getBookCategory(),
                aBk.getPublishingHouse(),
                aBk.getPublicationDate(),
                aBk.getPages(),
                aBk.getAvailable()
            });
        }
    ...
    private void displayBookCategories(){
        categorymodel = (DefaultTableModel) BookCategoryTable.getModel();
        categorymodel.setRowCount(0);
        
        for (BookCategory aCtgry : bookCategoryList) {
            categorymodel.addRow(new Object[]{
                aCtgry.getCategoryCode(),
                aCtgry.getCategoryName()
            });
        }
    }
    ...
    public void resetBookCategoryFields(){
        CategoryCodeTextField.setText(null);
        categoryNameTextField.setText(null);
    }
    
    private void updateBookCategoryComboBox(){
        bookCategoryComboBox.removeAllItems();

        bookCategoryList.forEach((BookCategory bctg)->{
            bookCategoryComboBox.addItem(bctg.getCategoryName());
        });
    }
    ...
    private void saveButtonBookActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonBookActionPerformed
        ...
        book = new Book(bookId, booktitle, author, bookCategory, publishingHouse, publicationDate, pages, available);
    ...
    private void saveButtonBookCategoryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonBookCategoryActionPerformed
        ...
        bookCategory = new BookCategory(bookCategoryCode, bookCategoryName, bookList);
    ...
    private void BookTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_BookTableMouseClicked
        try {
            ...
            bookIdTextField.setText(bookmodel.getValueAt(selectedRow1, 0).toString());
            titleTextField.setText(bookmodel.getValueAt(selectedRow1, 1).toString());
            authorTextField.setText(bookmodel.getValueAt(selectedRow1, 2).toString());
            bookCategoryComboBox.setSelectedItem(bookmodel.getValueAt(selectedRow1, 3).toString());
            publicationHouseTextField.setText(bookmodel.getValueAt(selectedRow1, 4).toString());
            java.util.Date date = new SimpleDateFormat("yyyy-MM-dd").parse((String)bookmodel.getValueAt(selectedRow1, 5).toString());
            publicationDateChooser.setDate(date);
            pagesTextField.setText(bookmodel.getValueAt(selectedRow1, 6).toString());
            availableComboBox.setSelectedItem(bookmodel.getValueAt(selectedRow1, 7).toString());
    ...
    private void BookCategoryTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_BookCategoryTableMouseClicked
        ...
        CategoryCodeTextField.setText(categorymodel.getValueAt(selectedRow2, 0).toString());
        categoryNameTextField.setText(categorymodel.getValueAt(selectedRow2, 1).toString());
